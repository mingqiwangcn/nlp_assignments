...
class BinaryClassifier(nn.Module):
    ...
    def compute_multi_hidden(self, embeds):
    ...
    def forward(self, batch_word_idxs):
        ...
        for word_idxs in batch_word_idxs:
            ...
            multi_hidden_lst = []
            for i in range(len(multi_embeds)):
                multi_hidden = self.compute_multi_hidden(multi_embeds[i])
                multi_hidden_lst.append(multi_hidden)
            hidden = torch.cat(multi_hidden_lst)
