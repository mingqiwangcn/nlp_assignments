...
class BinaryClassifier(nn.Module):
    def __init__(self, embedding_dim, corpus_size):
        ...
        embeds_q = np.random.uniform(-1.0, 1.0, (corpus_size, embedding_dim))
        ts_embeds_q = torch.tensor(embeds_q, dtype = torch.double)
        self.word_embeddings_q = nn.Embedding.from_pretrained(ts_embeds_q, freeze = False)
        
        embeds_k = np.random.uniform(-1.0, 1.0, (corpus_size, embedding_dim))
        ts_embeds_k = torch.tensor(embeds_k, dtype = torch.double)
        self.word_embeddings_k = nn.Embedding.from_pretrained(ts_embeds_k, freeze = False)
        
        embeds_v = np.random.uniform(-1.0, 1.0, (corpus_size, embedding_dim))
        ts_embeds_v = torch.tensor(embeds_v, dtype = torch.double)
        self.word_embeddings_v = nn.Embedding.from_pretrained(ts_embeds_v, freeze = False)
    ...
    def forward(self, batch_word_idxs):
        ...
        for word_idxs in batch_word_idxs:
            embeds_q = self.word_embeddings_q(word_idxs)
            embeds_k = self.word_embeddings_k(word_idxs)
            embeds_v = self.word_embeddings_v(word_idxs)
            MQ = embeds_q.mm(self.w_q)
            MK = embeds_k.mm(self.w_k)
            MV = embeds_v.mm(self.w_v)
