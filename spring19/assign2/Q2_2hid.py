...
class BinaryClassifier(nn.Module):
    def __init__(self, embedding_dim, corpus_size):
        ...
        ts_attend_u_2 = ts_attend_u * (-1.0)
        self.attend_u_2 = nn.Parameter(ts_attend_u_2);
        
        ts_attend_para = torch.ones(2, dtype = torch.double)
        
        self.attend_para = nn.Parameter(ts_attend_para)
    ...
    def forward(self, batch_word_idxs):
        ...
        for word_idxs in batch_word_idxs:
            ...
            for i in range(sent_len):
                dist0 = self.cosine(self.attend_u, embeds[i]).reshape(1,1)
                dist1 = self.cosine(self.attend_u_2, embeds[i]).reshape(1,1)
                dist_lst.append(dist1 * self.attend_para[0] + dist1 * self.attend_para[1])
