...
class Embedding2Hidden(nn.Module):
    def __init__(self):
        super(Embedding2Hidden, self).__init__()
        self.linear1 = nn.Linear(EMBEDDING_DIM, 32).double()
        self.linear2 = nn.Linear(32, EMBEDDING_DIM).double()
    
    def forward(self, embeds):
        hid_embeds = F.relu(self.linear1(embeds.double()))
        hidden = F.relu(self.linear2(hid_embeds))
        return hidden
...
class BinaryClassifier(nn.Module):
    def __init__(self, embedding_dim, corpus_size):
        ...
        self.embed2hid = Embedding2Hidden()
    ...
    def forward(self, batch_word_idxs):
        ...
        for word_idxs in batch_word_idxs:
            ...
            for i in range(sent_len):
                embeds_hid = self.embed2hid(embeds[i])
                dist = self.cosine(self.attend_u, embeds_hid).reshape(1,1)
